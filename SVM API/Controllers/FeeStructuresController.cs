...
namespace SVM_API.Controllers
{
    ...
    [FromQuery] int sessionId)
        ...
        [HttpGet("StudentFee/{studentId}")]
        public async Task<IActionResult> StudentFee(int studentId)
        {
            ...
            var fee = await _context.FeeStructures
                .FirstOrDefaultAsync(x =>
                    x.SessionId == student.SessionId &&
                    x.ClassId == student.ClassId);

            if (fee == null)
                return NotFound();

            bool alreadyPaid = await _context.FeePayments
            ...
            return Ok(new
            {
                ...
                OtherActivityFees = fee.OtherActivityFees,
                ComputerFees = fee.ComputerFees,
